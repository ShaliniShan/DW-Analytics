	...
	$(document).ready(function () {
	  ...
	  $(document).on({
		...
		ajaxStop: function () {
			...
			//	alert("Nothing to report!!! The event has not started yet. Please come back later");
				$("#home_row1").empty();
				$("#home_row2").empty();
				$("#home_row3").empty();
			} else {
	   ...
	   $(function() {
		...
		<?php
			...
			if (isset($_GET['specific']) && $parseAPI->isBeacon($_GET['specific'])) {
				...
				//echo var_dump($info['beacon_graph']);
				echo "$(function() {
			...
			else if ($_GET['specific'] == 'Visitors') {
				echo "$(function() {
				    tableFill.selectFill('exhibitor-hourly', ". $info['beacon_graph'] .");
				});";
			} 
