var Error_checks = (function () {
	...
	function checkInjectionForBeacon(checkValue) {
		var isPossibleOfInjection = false;
		var regex =/^[a-z0-9_]+$/;;
		var check = regex.exec(checkValue);
		isPossibleOfInjection = (!check ? false : true);
	...
	function checkEventCode(checkVal) {
		var checkErrorCode = false;
		var checkValToLower = checkVal.toLowerCase();
		//var regex = /^[a-z0-9\_]+$/;
		var regex = /^[a-z0-9_]+$/;
		var check = regex.exec(checkValToLower);
		checkErrorCode = (!check ? false : true);
	...
	function generateAddEventCheck() {
		//$("#submit_event").submit(function (event) {
			...
			if ($code === '') {
				...
				// if event code exists in the document.add_event_name.action then it is
				// coming from edit event page, otherwise, it is from new event
				//var isUniqueCodeCheckNeeded = true;
				//if ($formAction.indexOf($code) !== -1) {
					//isUniqueCodeCheckNeeded = false;
				//}
				
				var isValidCode = checkEventCode($code);
				if (!isValidCode){
					$formError += "<span class='glyphicon glyphicon-exclamation-sign' aria-hidden='true'></span>" +
					"<span class='sr-only'>Error:</span>" +
					" Invalid Characters in Event Code, Allowed characters - [a-z, 0-9, _]<br/>";
				} else {
					var $editEvtId = null;
					if (document.getElementById('editEventId')) {
						//This input element is set only in the edit specific details for event
						$editEvtId = document.getElementById("editEventId").value;
					}
					//if (isUniqueCodeCheckNeeded) {
						/*
						...
						$.ajax({
							...
							data: {action1: 'isEventCodeUnique', params: [$code, $editEvtId]},
							async: false,
					...
					//}
					var $action = "/Organizer/admin/edit/" + $code.toLowerCase();
	...
	function generateAddExhibitorCheck() {
		...
		$email = $("#exhibitor_email").val().trim(),
		$error = '<div class="alert alert-danger alert-dismissable">',
		$formError = "";
	if($name === '') {
		$formError += "<span class='glyphicon glyphicon-exclamation-sign' aria-hidden='true'></span>" +
	...
	else if(checkInjectionForBeacon($name)) {
		$formError += "<span class='glyphicon glyphicon-exclamation-sign' aria-hidden='true'></span>" +
                   "<span class='sr-only'>Error:</span>" +
                   " Invalid Characters in Beacon name<br/>";
	}
