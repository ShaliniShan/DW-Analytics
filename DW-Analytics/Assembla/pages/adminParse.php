	...
	class AdminParseAPI {
		...
		**/
			...
			//	$name = $current_event->get("name");
	           ...
	           // echo var_dump($_GET['action']);
				...
				$html .= $table_row
						...
						."<td>". "<a href='/Organizer/Admin/Edit/". $current_event->get("eventCode") ."' onclick='stopPropagation(event)'><i class='fa fa-edit'> </i>Edit</a>"
						."</tr>";
			}
			return $html;
		}
		...
		public function buildSpeakersAtEvent($event) {
			...
			$query->equalTo("eventId", $eventId);
			$results = $query->find();
			$html = "<table id='records-speakers' class='table table_sortable table_pick datatable display table-hover'>";
				...
				foreach($results as $speaker) {
					...
					$html .= "<tr><td><img src='$avatar' alt='' width='50'></td>"
						...
						."<td>". $speaker->get("title") ."</td>"
							...
							. "</tr>";*/
					$tweet = $speaker->get("twitterURL")? $speaker->get("twitterURL"):"";
					$linkedin = $speaker->get("linkedInURL")? $speaker->get("linkedInURL"):"";
					$event_speaker_id = $eventId . "/speaker_" . $id;
					$html .= "<tr><td>" . $speaker->get("firstName") . " " . $speaker->get("lastName")."</td>"
					        ."<td>". $speaker->get("company") ."</td>"
					        ."<td>". $speaker->get("title") ."</td>"
					        ."<td><a target='_blank' href=".$tweet."></a></td>"
					        ."<td><a target='_blank' href=". $linkedin ."></a></td>"
					         //."<td>". $speaker->get("linkedInURL") ."<i class='fa fa-linkedin'> </i></td>"
					        ."<td><a href='/Organizer/Admin/" . $event_speaker_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
					        ."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"
					        . "</tr>";
		...
		public function buildSessionsAtEvent($event) {
			...
			if(count($results) > 0) {
				foreach($results as $session) {
					...
					$event_session_id = $eventId . "/session_" . $id;
					$html .= "<tr>"
							...
							."<td>". $session->get("endTime")->format("Y-m-d H:i:s") ."</td>"
							."<td><a href='/Organizer/Admin/" . $event_session_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
							."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"
							."</tr>";
					}
					$tags_added = 1;
	...
	public function buildExhibitorAtEvent($event) {
			...
			if(count($results) > 0) {
				foreach ($results as $exhibitor) {
					...
					$event_exhibitor_id = $eventId . "/exhibitor_" . $id;
					...
					$html .= "<tr>"
							...
							."<td><a href='/Organizer/Admin/" . $event_exhibitor_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
							."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"	
							."</tr>";
				}
				$tags_added = 1;
		...
		public function buildBeaconAtEvent($event) {
			...
			//$html = "<table class='table table_sortable'>";
			$html .= "<thead><tr><th class='table-header'>Beacon Name</th>"
			."<th class='table-header'>Beacon Type</th>"
			."<th class='table-header'>Edit</th>"
			...
			if(count($results) > 0) {
				foreach ($results as $beacon) {
					...
					$event_beacon_id = $eventId . "/beacon_" . $id;
					...
					."<td><a href='/Organizer/Admin/" . $event_beacon_id ."><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
					."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"
					."</tr>";
					/*	} else {
						$html .= "<tr>"
		...
		public function buildNotificationAtEvent($event) {
			...
			if(count($results) > 0) {
				foreach ($results as $not) {
					...
					$event_not_id = $eventId . "/notification_" . $id;
 					$html .= "<tr>"
							."<td>". $not->get("title") ."</td>"
							."<td>". $this->getBeaconName($not->get("beaconId")) ."</td>"
							."<td>". $not->get("triggerTimeInSecs") ."</td>"
							//."<td>". $message ."</td>"
							."<td><a target='_blank' href='". ($pdf ? $pdf->getURL() : ' ') ."'></a>"."</td>"
							."<td><a target='_blank' href='".$url."'></a>"."</td>"
							."<td><a href='/Organizer/Admin/" . $event_not_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
							."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"	
							."</tr>";
				}
				$tags_added = 1;
		...
		public function buildSurveyAtEvent($event) {
			...
			$html .= "<thead><tr><th class='table-header'>Title</th>"
					...
					."<th class='table-header'>Number of Questions</th>"
					."<th class='table-header'>Edit</th>"
					."<th class='table-header'>Delete</th>
					</tr></thead><tbody>";
			if(count($results) > 0) {
				foreach ($results as $survey) {
					...
					$event_survey_id = $eventId . "/survey_" . $id;
					...
					$html .= "<tr>"
							...
							."<td>". $session_details->get("title") ."</td>"
							."<td>". count($quest_survey) ."</td>"
							."<td><a href='/Organizer/Admin/" . $event_survey_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
							."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"	
							."</tr>";
				}
				$tags_added = 1;
			...
			$html .= '</tbody></table>';
			if ($tags_added) {
				$html .= "<form action='/Organizer/Admin/Edit/" . $_GET[action] ."' method = 'POST'><button name='add_survey_details' id='addSurvey' href='javascript:;' type='submit' class='btn btn-custom btn-sm btn-block'>Add Survey</button></form>";
			}
			return $html;
		}
		public function buildSponsorsAtEvent($event) {
			...
			if(count($results) > 0) {
				foreach($results as $sponsor) {
					...
					$event_sponsor_id = $eventId . "/sponsor_" . $id;
					$html .= "<tr><td>" . $sponsor->get("name") ."</td>"
					."<td>". $sponsor->get("level") ."</td>"
					."<td>". $sponsor->get("website") ."</td>"
					."<td><a href='/Organizer/Admin/" . $event_sponsor_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
					."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"
					. "</tr>";
		...
		public function buildContentAtEvent($event) {
			...
			if(count($results) > 0) {
				foreach ($results as $content) {
					...
					$event_content_id = $eventId . "/content_" . $id;
					$html .= "<tr>"
					."<td>". $content->get("title") ."</td>"
					."<td><a target='_blank' href='". ($pdf ? $pdf->getURL() : '') ."'></a>"."</td>"
					."<td><a target='_blank' href='".$url ."'></a>"."</td>"
					."<td><a href='/Organizer/Admin/" . $event_content_id ."'><i class='fa fa-plus-square-o'></i>  Edit</a></td>"
					."<td><a class='delete_chosen' data-info='$id' data-info2='$table' data-toggle='modal' data-target='#deleteModal' href=''><i class='fa fa-trash'></i>  Delete</a></td>"
					."</tr>";
				}
				$tags_added = 1;
  ...
  //echo var_dump($timezones);
    ...
    public function displayAddedSpeakers($eventId = null, $eventCode = null) {
		...
		/**
		  ...
		  */
		public function buildNotificationSelect($eventId) {
		...
		public function buildExhibitorSelection($selected=null) {
			...
			$html = '';
			foreach($results as $exhibitor) {
				$select = $selected == $exhibitor->getObjectId() ? 'selecte`d>' : ">";
				$html .= '<option value="' .$exhibitor->getObjectId().'"' . $select . $exhibitor->get("name") . '</option>';
			}
			return $html;
		}
		
		/* Submission Functions */
		public function submitNewEvent($admin, $title, $timezone, $event_code, $logo, $location, $address, $address2, $city, $state, $zip, $country, $startTime, $endTime, $hash) {
			//echo var_dump($title);
		    ...
		    return $event_submit;
		    //echo var_dump($event_submit->getObjectId());
		}
		...
		public function submitNewSponsor($eventId, $name, $profile, $level,$website,$logo) {
			//$event_info = $this->getEventInfo($event);
		...
		public function submitNewSession($event, $title, $desc, $startTime, $endTime, $speaker) {
			$eventId = $this->eventId;
		...
		public function submitNewExhibitor($event, $name, $desc, $booth, $website, $logo) {
			$eventId = $this->eventId;
		...
		public function submitNewContent($eventId, $title, $description, $PDF, $url) {
			//$eventId = $this->eventId;
		...
		public function submitNewSurvey($eventId, $name, $session, $questions) {
			//$eventId = $this->eventId;
