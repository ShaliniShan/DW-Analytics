...
<?php 
	...
	**/
	...
	class AdminParseAPI {
	  ...
	  private function getParseUserByEmail($email) {
	  	$users = ParseUser::query();
	  	$users->equalTo("email", $email);
	 ...
	 /* public function getSurveyQuestionById($sponsorId) {
		...
		public function validateUserByHash($hash) {
			$verification_hash_array = array();
			$user_found = null;
			...
			foreach($users_db as $user_rec){
				
				$verification_hash_array = $user_rec->get("verificationHash");
				//if ($verification_hash_array) {
					//echo var_dump($verification_hash_array);
				//}
				for($x=0; $x<count($verification_hash_array); $x++){
					if ($verification_hash_array[$x] == $hash) {
					    $user_found = $user_rec;
					    break;	
					}
				}
				if ($user_found) {
					...
					/*if ($_SESSION['validatedExhibitorId'] &&
					$_SESSION['validatedExhibitorId'] != $hash) {
						$exhibitor_user = new ParseObject("UserExhibitorRoles");
						$exhibitor_user->set("UserId", $user_found->getObjectId());
						$exhibitor_user->set("ExhibitorId", $hash);
		...
		public function saveUserExhRecordIfNotExist($userId, $exhibitorId, $eventId) {
			/*
			 * check if record exists for userid and exhibitorid combination
			 */
			...
			if ($userExhibitor) {
				...
				//create record
				$exhibitor_user = new ParseObject("UserExhibitorRoles");
				$exhibitor_user->set("UserId", $userId);
				$exhibitor_user->set("ExhibitorId", $exhibitorId);
				$exhibitor_user->set("EventId", $eventId);
	  ...
	  private function getEventInfoForEdit($id) {
	   ...
	   private function getBeaconInfo($id) {
	    ...
	    private function getParentBeaconByUuid ($uuid) {
	    	$query = $this->parseQuery("Beacon");
		...
		private function getAllEventsForExhibitor($exhibitor) {
			$events_array = array();
			$exh_event = $this->parseQuery("UserExhibitorRoles");
			$exh_event->equalTo("UserId", $exhibitor);
			$exhibitor_objects = $exh_event->find();
			foreach ($exhibitor_objects as $exhibit) {
				$exh_id = $exhibit->get("ExhibitorId");
				//use exhibitor id as input to get event id
				$exhibit_event = $this->parseQuery("Exhibitor");
				$exhibit_event->equalTo("objectId", $exh_id);
				$exhibit_event->limit(1);
				$exhibit_event_info = $exhibit_event->first();
				$exh_event_id = $exhibit_event_info->get("eventId");
				$event_info = $this->getEventInfo($exh_event_id);
				array_push($events_array, $event_info);
			}
			return $events_array;
		}
		...
		**/
		 ...
		 * validate functions
		 */
		public function getEventByCode($eventCode, $editEventId) {
			$numRows = 0;
			$query = $this->parseQuery("Event");
			$query->equalTo("eventCode", $eventCode);
			$event_rows = $query->find();
			//This is edit event scenario, where same event code 
			//may or may not be passed. So, in that case, if we 
			//find event record with the same event id while edit
			//we should return 0
			
			$numRows = count($event_rows);
			if ($editEventId && count($event_rows) == 1) {
				foreach ($event_rows as $evt) {
					if (($evt->getObjectId() == $editEventId) &&
					    ($evt->get("eventCode") == $eventCode)) {
					    $numRows = 0;
					    break;    	
					}
				}
		...
		public function buildEventEditTableForExhibitor($exhibitor) {
			$allEvents = $this->getAllEventsForExhibitor($exhibitor);
			$html = "";
			for ($i = 0; $i < count($allEvents); $i++) {
				$current_event = $this->getEventInfo($allEvents[$i]->getObjectId());
				$event_id = $current_event->getObjectId();
		...
		public function buildEventEditDisplay($event) {
			...
			$html = "<table id='eventInfo' class='table compact table-hover' >";
	...
	public function buildSpeakersAtEvent($event, $isAddBtnNeeded = true) {
			...
			$html = "<table id='speakersList' class='table table_sortable table_pick datatable display compact table-hover no-border'>";
			
			$html .= "<thead><tr class='no-border'><th class='table-header'>Name</th>"
			...
			if(count($results) > 0) {
				...
				if($speaker->get("twitterURL")){
					...
					$id_table = $id . ':' . $table; 
					$html .= "<tr class='no-border deleteRec' data-id=$id_table><td>" . $speaker->get("firstName") . " " . $speaker->get("lastName")."</td>"
					        ."<td>". $speaker->get("company") ."</td>"
					        ."<td>". $speaker->get("title") ."</td>"
		...
		public function buildSessionsAtEvent($event) {
			...
			$html = "<table id='sessionsList' class='table table_sortable table_pick datatable display table-hover no-border'>";
			$html .= "<thead><tr class='no-border'><th class='table-header'>Title</th>"
			...
			if(count($results) > 0) {
                ...
                $newEndTime = date('h:i A', strtotime($currentEndTime));
					...
					$html .= "<tr class='no-border deleteRec' data-id=$id_table>"
	...
	public function buildExhibitorAtEvent($event) {
			...
			$html = "<table id='exhibitorsList' class='table table_sortable table_pick datatable display table-hover no-border'>";
		...
		public function buildBeaconAtEvent($event) {
			...
			$html = "<table id='beaconsList' class='table table_sortable table_pick datatable display table-hover no-border'>";
		...
		public function buildNotificationAtEvent($event) {
			...
			$html = "<table id='triggeredContentList' class='table table_sortable table_pick datatable display table-hover no-border'>";	
			$html .= "<thead><tr class='no-border'><th class='table-header'>Title</th>"
		...
		public function buildSurveyQuestionsAtEvent($event) {
			...
			$html = "<table id='surveysList' class='table table_sortable table_pick datatable display table-hover no-border'>";
			$html .= "<thead><tr class='no-border'>"
		...
		public function buildSponsorsAtEvent($event) {
			...
			$html = "<table id='sponsorsList' class='table table_sortable table_pick datatable display table-hover no-border'>";
			$html .= "<thead><tr class='no-border'><th class='table-header'>Name</th>"
			...
			if(count($results) > 0) {
				foreach($results as $sponsor) {
					...
					$id_table = $id . ':' . $table;
					$html .= "<tr class='no-border deleteRec' data-id=$id_table><td>" . $sponsor->get("name") ."</td>"
		...
		public function buildContentAtEvent($event) {
			...
			$html = "<table id='contentsList' class='table table_sortable table_pick datatable display table-hover no-border'>";
			$html .= "<thead><tr class='no-border'><th class='table-header'>Title</th>"
			."<th class='table-header'>PDF</th>"
			."<th class='table-header'>URL</th>"
			."<th class='table-header'>Edit</th>"
    ...
    public function getTwitterAccount($twitter){
        ...
        */
		...
		public function buildSessionSelection($selected = null) {
			...
			//$html = '';
			foreach($results as $session) {
				if ($selected != null && ($session->get("beaconId")) != null){
				$select = $selected == $session->get("beaconId") ? "selected>" : ">";
				$html .= '<option value="' .$session->getObjectId() .'"' . $select . $session->get("title") . '</option>';
			}else{
				//$select = $selected == $session->getObjectId() ? "selected>" : ">";
				$html .= '<option value="' .$session->getObjectId().'"' . ">" . $session->get("title") . '</option>';
			}}
				//$select = $selected == $session->getObjectId() ? "selected>" : ">";
				//$html .= '<option value="' .$session->getObjectId().'"' . $select . $session->get("title") . '</option>';
			return $html;
		...
		public function submitNewEvent($exist_obj_id = NULL,$admin, $title, $timezone, $event_code, $logo, $location, $address, $address2, $city, $state, $zip, $country, $startTime, $endTime, $hash) {
			...
			$event_code_to_lower = strtolower($event_code);
		//	$event_submit = new ParseObject("Event");
		    $event_submit->set("name", $title);
		    $event_submit->set("timeZone", $timezone[0]);
		    $event_submit->set("eventCode", $event_code_to_lower);
		...
		public function submitNewExhibitor($exist_obj_id = NULL,$event, $name, $email, $desc, $booth, $website, $logo) {
		if ($exist_obj_id) {
			...
			if ($email) {
				//if email exists, then need to add this exhibitor as user
				//in the user table
				if (!$exist_obj_id) {
					$roles_array = array();
					$verification_hash_array = array();

					//check if user has to be created or not, what if
					//user is existing? Use email to verify that from
					//the UserExhibitorRoles table
					//$user_exhibitor_role_obj = $this->getUserExhibitorInfoByEmail($email);
					//echo var_dump($email);
					$users_existing = $this->getParseUserByEmail($email);
					//echo var_dump($users_existing);
					if (count($users_existing) == 0) {
						$user_exhibitor = new ParseUser();
						$user_exhibitor->set("company", $name);
						$user_exhibitor->set("email", $email);  
						$user_exhibitor->set("username", $name);
						$user_exhibitor->set("password", $name);
						$user_exhibitor->set("isVerified", False);
						//$exh_role_id = "e" . ":" . $exhibitor_actual->getObjectId();
						//array_push($roles_array, $exh_role_id);
						array_push($roles_array, "a");
						$user_exhibitor->setArray("roles", $roles_array);
						array_push($verification_hash_array, $exhibitor_actual->getObjectId());
						$user_exhibitor->setArray("verificationHash", $verification_hash_array);
						$user_exhibitor->signUp();
					} else {
						//Add verfication hash (exhibitor obj id) to the array
						//for that user record and save
						$user_update = $users_existing[0];
						
						$verification_hash_array = $user_update->get("verificationHash");
						array_push($verification_hash_array, $exhibitor_actual->getObjectId());
						$user_update->setArray("verificationHash", $verification_hash_array);
						//echo var_dump($user_update);
						try {
							$user_update->save(true);
						} catch(ParseException $ex) {
							echo $ex->getMessage() . ' ' . $ex->getCode();
						}
					}
					...
					//After saving exhibitor as user, get the email address of Organizer
					//Get organizer from current event
					$event_obj = $this->getEventInfoById($event);
					$event_name = $event_obj->get("name");
					//$event_organizers = $event_obj->get("organizers");
					//$organizer_id = '';
					//foreach($event_organizers as $org) {
					//$organizer_id = $org;
					//}
					//$query = ParseUser::query();
					//$query->equalTo("objectId", $event_organizers[0]);
					//$user_organizer = $query->find();
					//echo var_dump($user_organizer);
					//echo var_dump($user_organizer->get);
					$from = $_SESSION['userEmail'];
					$message = "Hello from Event Organizer";
					$to = $email;
					$subject="Activation Code For event";

				//	$body='Your Activation Code is '.$name;
					$body=''.$_SESSION['username'].' has added you as an Exhibitor to '. $event_name.'
					Please click on the link below to activate. 
					http://cms.beepetc.com/Organizer/admin/activate/'. $exhibitor_actual->getObjectId().'
					Please follow the instructions to create and/or activate your account. 
					Once activated, you will have access to visitor and engagement metrics for your booth.
					';
					$headers = "From:".$from;
		...
		public function submitNewBeacon($exist_obj_id = NULL,$event, $name, $beacon_type, $beacon_for) {
			...
			//echo var_dump($beacon_for[0]);
			if ($exist_obj_id) {
				$beacon_for_val = $beacon_for;
				$beacon = $this->getBeaconInfo($exist_obj_id);
				
				//undo the previous beacon type
				if ($beacon) {
					if ($beacon->get("companyId") != NULL ) {
						//it is exhibitor beacon, new company id
						//will be stored again below as part of 
						//this edit, hence marking it as null
						$beacon->set("companyId", '');
				/*		try{
							$beacon->save();
						} catch (ParseException $ex) {
							echo $ex->getMessage() . ' ' . $ex->getCode();
						...
						//it could be session or organizer
						$query_session = $this->parseQuery("Session");
						$query_session->equalTo("beaconId", $beacon->getObjectId());
						$sessions = $query_session->find();
						if ($sessions) {
							//beacon type is session
							foreach($sessions as $sess) {
								//set beaconId as null
								$sess->set("beaconId", '');
								try {
									$sess->save();
								}catch (ParseException $ex) {
						...
						//for organizer, nothing to do
					}
				}
				
			} else {
				$beacon_for_val = $beacon_for[0];
			...
			*/
			...
			if($beacon_type == 'Organizer') {
				...
				$beacon->set("companyId", $beacon_for_val);
				try {
					$beacon->save();
					if ($availableBeacon) {
						$availableBeacon->set("IsAvailable", false);
				...
				//echo var_dump($beacon_for);
				$query = $this->parseQuery("Session");
				$query->equalTo("objectId", $beacon_for_val);
				$query->limit(1);
				$session = $query->first();
						
				//$beacon->set($session, $beacon_for[0]);
				...
				try {
					...
					if ($availableBeacon) {
		          	...
		          	if ($exist_obj_id) {
		          	    $session->set("beaconId", $exist_obj_id);	
		          	} else {
		        	    $session->set("beaconId", $beaconid);
		          	}
				    $session->save();			
				} catch (ParseException $ex) {
					echo $ex->getMessage() . ' ' . $ex->getCode();
	...
	/*		for ($i=0; $i < count($questions); $i++) { 
		...
		public function deleteRow($id, $class) {
			...
			if($this->getSelectedTable($class) == 'Company') {
				...
				$query->equalTo("objectId", $id);
				$results = $query->first();
				//get uuid of beacon if class is beacon
				if ($this->getSelectedTable($class) == 'Beacon') {
				...
				if ($this->getSelectedTable($class) == 'Beacon') {
					//By now, the actual beacon record for event would be deleted
					//Get parent row of beacon with uuid and mark isAvailable as true.
					//parent row is one with event id not set
					$parent_beacon = $this->getParentBeaconByUuid($uuid);
				    $parent_beacon->set("IsAvailable", true);
				    $parent_beacon->save();
