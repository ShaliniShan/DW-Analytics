<?php 
    ...
    class Authenticator{
        ...
        public function getRegisterUserId() {
        ...
        public function submitNewExhibitorUser($username, $pass1, $pass2) {
        	if($this->isUsernameTaken($username)) {
        		$this->HandleError("Username is taken");
        		return $this->getErrorMessage();
        	} else {
        	...
        	//	echo var_dump($this->getRegisterUserId());
			    ...
			    if (!$isExhRoleSet) {
			    	$roles_array_new = array();
			    	array_push($roles_array_new, "e");
			    	foreach ($roles_array as $role) {
			    		array_push($roles_array_new, $role);
			    	}
			    	$user->setArray("roles", $roles_array_new);
			    }
